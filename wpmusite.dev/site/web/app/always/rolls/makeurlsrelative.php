...
add_filter( 'http_request_args', __NAMESPACE__ . '\\preempt_expect_header', 10, 3 );
function preempt_expect_header($r) {
    $r['headers']['Expect'] = '';
    return $r;
}

// This should have been solved by apache/nginx rewrite, but we need to make it simpler
add_filter('network_site_url', __NAMESPACE__ . '\\fix_site_url_location', 10, 3);
function fix_site_url_location( $url, $path, $scheme ) {
    $urls_to_fix = array(
        '/wp-admin/network/',
        '/wp-login.php',
        '/wp-activate.php',
    ...
    foreach( $urls_to_fix as $maybe_fix_url ) {
        $fixed_wp_url = '/core' . $maybe_fix_url;
