<?php
/**
* Plugin Name: Base Module Auto Loader
* Description: This plugin is designed to autoload rolls modules and deactivate conflicting plugins.
...
class RollModuleLoader {
    ...
    public static function get_instance() {
        if ( !self::$instance ) {
            ...
            if ( did_action( 'plugins_loaded' ) ) {
                ...
                //add_action( 'admin_init', array( __CLASS__, 'add_rolls2muplugin' ) );
            }
            self::load_basemodules();
            self::add_rolls2muplugin();
        }
    ...
    private static function get_basemodules() {
        ...
        if ( $plugins_dir = @opendir( WPMU_PLUGIN_DIR . '/rolls' ) ) {
    ...
    public static function plugins_textdomain() {
        load_plugin_textdomain( 'rollmodules', false, WPMU_PLUGIN_DIR . '/langs/' );
    }
    ...
    public static function load_basemodules() {
        if ( ! empty(self::$basemodules) ) {
            foreach ( self::$basemodules as $plugin_file ) {
                if ( !is_readable( WPMU_PLUGIN_DIR . '/rolls/' . "$plugin_file" ) )
                                continue;
                require_once( WPMU_PLUGIN_DIR . '/rolls/' . "$plugin_file" );
            }
        }
    }


    private static function add_rolls2muplugin() {
        if ( ! empty(self::$basemodules) ) {
            add_action('admin_init', function() {
                add_action('after_plugin_row_mu-require.php', function() {
                    $table = new WP_Plugins_List_Table;
                    foreach ( self::$basemodules as $plugin_file ) {
                        $plugin_data = get_plugin_data( WPMU_PLUGIN_DIR . '/rolls/' . "$plugin_file", false);
...
$RollModuleLoader = RollModuleLoader::get_instance();
